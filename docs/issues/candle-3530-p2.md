...
- **Context:** Sempervictus replied to [p1](candle-3530-p1.md) without sharing the repo, but his answer plus the crash log fingerprint (hybrid Mamba allocation, 28 slots, 36 linear-attention layers, NVFP4 on Spark / SM121) lets us identify the model with high confidence. Running `hf-fm inspect --dtypes` on the candidate gives ground-truth on-disk numbers that **correct his "FP4 = ½ × param count" heuristic** (actual is ~47.24 GiB, not ~40 GiB) and **close branch B of p1** (mmap-then-copy / 2× bounds counting): the 47.24 GiB model + 18 GiB KV pool + 2 GiB Mamba state + ~14 GiB runtime accounts for `used: 81 GB` as a single, honest accounting. The bug locus narrows to what sempervictus himself surfaced: the KV allocator caps at 18 GB on SM121 while 38 GB of UMA sits genuinely free.
- **Outcome:** —
- **Lesson / Leverage angle:** First reply in the archive that demonstrates `inspect --dtypes` on a real NVFP4 model and *names* a specific public repo with high confidence from a crash log alone. The ~30%-overhead-on-top-of-FP4-bulk fact (block-scaling factors + BF16 norms + tiny F32 scalars adding up to ~1.3× the FP4 weight bytes) is a general NVFP4 truth worth carrying forward to other replies. The identification reasoning chain — fingerprinting an architecture from a vllm-rs allocator log — is reusable.

---

I went looking for the repo since your hypothesis turns on the actual on-disk size, and the fingerprint in your crash log is narrow enough to identify the model with reasonable confidence:
...
Intersection: **`nvidia/Qwen3-Next-80B-A3B-Instruct-NVFP4`** (or its RedHat repackage `RedHatAI/Qwen3-Next-80B-A3B-Instruct-NVFP4` — same weights). Running `hf-fm inspect <repo> --dtypes` against the NVIDIA repo's safetensors headers (HTTP Range, no weight data downloaded):
...
Hoping this sharpens the picture. If `nvidia/Qwen3-Next-80B-A3B-Instruct-NVFP4` isn't the exact repo (e.g., you're running a private build or the RedHat repackage), the on-disk numbers will differ by a few percent at most; the math above is robust to that.
