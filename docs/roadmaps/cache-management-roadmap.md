...
| Doc clarifications | Cheap correctness sweep across the reference docs: (a) document `--filter` as case-**in**sensitive once the alignment above lands (clap help [src/bin/main.rs:444](../../src/bin/main.rs#L444) and [cli-reference.md](../cli-reference.md)); (b) add the missing `--show-cached` glyph legend (`✓ complete · partial · ✗ missing`) and the `du` `●` partial-marker legend to [cli-reference.md](../cli-reference.md) — both are legended in the README but absent from the reference; (c) narrow the README `status` row's "complete / partial / missing / excluded" wording to note that four-state vocabulary is the per-repo `status <REPO_ID>` view — the all-repos table prints only `ok` / `PARTIAL` ([src/bin/main.rs:2334](../../src/bin/main.rs#L2334)). |
...
### v0.10.7 — CLI parity & scriptability (planned, closes the v0.10 line)
...
| Reclaimed-bytes totals line on `cache gc` / `cache delete` | After an eviction the user's immediate next question is *"how much did I get back?"*, but the `cache gc` / `cache delete` mutation handlers print per-repo removals with no rollup. Add a `Reclaimed: X.XX GiB (N repos)` footer in the project's established totals-row style (matching `du`'s `total` footer and the dry-run's `Total:` line) — the number is already in hand because the dry-run plan computes it. Small, render-only. |
...
1. **Reclaimed-bytes totals line** — render-only footer on `cache gc` / `cache delete`; the number is already computed by the dry-run plan.
2. **`download-file --dry-run`** — resolve the file list and route to the existing `run_dry_run` renderer; no new rendering code.
3. **`diff --limit N`** — cap the three section loops after `--filter`, add the summary truncation note and the `truncated` JSON field mirroring `inspect`.
4. **`list-files --json`** — serialise the existing per-file row struct.
5. **`du --json` + `status --json`** — serialise the existing `RepoDiskUsage` / `FileStatus` structs; ship together.
