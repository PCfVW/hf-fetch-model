...
5. ⬜ **`du --json` + `status --json`** — serialise the existing `RepoDiskUsage` / `FileStatus` structs; ship together. **(next / pending)**

**Bonus (not originally planned):** ✅ **`test(json)`** (`c1c4d9b`) — replaced the smoke/substring `--json` assertions (and the absent `diff`/`info` ones) with real `serde_json`-parse-based schema + invariant tests across every `--json` output; added `serde_json` to dev-dependencies. Surfaced while building item 2's tests.
